...
bot = telebot.TeleBot(
    "5642140510:AAFmk2k0UDk6-nqMAQCQY3w1vL98x_bJ4zU", state_storage=state_storage
)
...
def start(m, res=False):
    ...
    button2 = types.InlineKeyboardButton(text="/lowprice", callback_data="button2")
...
def city(message):
    ...
    bot.set_state(
        message.from_user.id, User.city, message.chat.id
    )  # Состояние меняется на заполнение города
...
def city(message):
    ...
    bot.set_state(
        message.from_user.id, User.checkin, message.chat.id
    )  # Меняется состояние на календарь (дата заезда, но календарь не открывается)
...
def cal(c):
    ...
    if not result and key:
        ...
        # Далее перменные, хранящие в себе собранную инфу и передается далее в отдельную
        # функцию для отправки в RAPIDAPI
...
def user_info(message, checkin, checkout, city):
    text = hotels(
        dest_id(city), checkin, checkout
    )  # Перменная, храняющая текст в виде названия отелей, стоимость проживаний, собранную из функций hotels и dest_id
    bot.send_message(
        message.chat.id, text
    )  # отправляет в телеграм чат собранную инфу от RAPIDAPI
